...
}var $author$project$Main$init = {latexOut: '', mdIn: ''};
...
var $author$project$Main$renderCodeBlock = F2(
	function (codeBlock, code) {
		if (codeBlock.$ === 'Indented') {
			return code;
		} else {
			var isOpen = codeBlock.a;
			var fence = codeBlock.b;
			return '\\begin{lstlisting}' + (function () {
				var _v1 = fence.language;
				if (_v1.$ === 'Nothing') {
					return '\n';
				} else {
					var language = _v1.a;
...
var $author$project$Main$renderInline = function (inline) {
	switch (inline.$) {
		case 'Text':
			var str = inline.a;
			return str;
		...
		case 'Link':
			...
			return '\\href{' + (url + ('}{' + ($author$project$Main$renderInlines(inlines) + '}')));
		case 'Image':
			var src = inline.a;
			return 'img';
		case 'HtmlInline':
		...
		case 'Emphasis':
			var length = inline.a;
			var content = inline.b;
			switch (length) {
				case 1:
					return '\\textif{' + ($author$project$Main$renderInlines(content) + '}');
				case 2:
					return '\\textbf{' + ($author$project$Main$renderInlines(content) + '}');
				default:
					return $author$project$Main$renderInlines(content);
			}
		default:
			return 'custom';
	}
};
var $author$project$Main$renderInlines = function (inlines) {
	return $elm$core$String$concat(
		A2($elm$core$List$map, $author$project$Main$renderInline, inlines));
};
var $author$project$Main$renderHeading = F2(
	function (depth, content) {
		switch (depth) {
			case 1:
				return '\\section{' + ($author$project$Main$renderInlines(content) + '}');
			case 2:
				return '\\subsection{' + ($author$project$Main$renderInlines(content) + '}');
			case 3:
				return '\\subsubsection{' + ($author$project$Main$renderInlines(content) + '}');
			case 4:
				return '\\subsubsection{' + ($author$project$Main$renderInlines(content) + '}');
			default:
				return '';
...
var $author$project$Main$renderListBlock = F2(
	function (listBlock, contents) {
		var type_ = function () {
		...
		var header = function () {
			var _v0 = listBlock.type_;
			if (_v0.$ === 'Unordered') {
				return '\\begin{itemize}\n';
			} else {
				var start = _v0.a;
				return '\\begin{enumerate}\n\\setcounter{enumi}{' + ($elm$core$String$fromInt(start - 1) + '}\n');
			}
		}();
		var footer = '\\end{' + (type_ + '}\n');
		var content = $elm$core$String$concat(
			A2(
				$elm$core$List$map,
				function (line) {
					return '\\item ' + (line + '\n');
...
var $author$project$Main$renderBlock = function (block) {
	switch (block.$) {
		...
		case 'Heading':
			var level = block.b;
			var inlines = block.c;
			return A2($author$project$Main$renderHeading, level, inlines);
		case 'CodeBlock':
			var codeBlock = block.a;
			var code = block.b;
			return A2($author$project$Main$renderCodeBlock, codeBlock, code);
		case 'Paragraph':
			var content = block.b;
			return $author$project$Main$renderInlines(content);
		case 'BlockQuote':
			var content = block.a;
			return '\\begin{quote}\n' + ($author$project$Main$renderBlocks(content) + '\n\\end{quote}');
		case 'List':
			var listBlock = block.a;
			var contents = block.b;
			return A2(
				$author$project$Main$renderListBlock,
				listBlock,
				A2($elm$core$List$map, $author$project$Main$renderBlocks, contents));
		case 'PlainInlines':
			var inlines = block.a;
			return $author$project$Main$renderInlines(inlines);
		default:
			return 'custom';
	}
};
var $author$project$Main$renderBlocks = function (blocks) {
	return $elm$core$String$concat(
...
var $author$project$Main$generateLatex = function (content) {
	...
	return _Utils_ap(
		latexHeader,
		_Utils_ap(latexBody, latexFooter));
};
var $author$project$Main$update = F2(
	function (msg, model) {
		...
		return _Utils_update(
			model,
			{
				latexOut: $author$project$Main$generateLatex(newContent),
				mdIn: newContent
			});
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Enter markdown here')
							])),
						A2(
						$elm$html$Html$textarea,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$value(model.mdIn),
								$elm$html$Html$Events$onInput($author$project$Main$Change),
								$elm$html$Html$Attributes$rows(25),
								$elm$html$Html$Attributes$id('md'),
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Get LaTeX here')
							])),
						A2(
						$elm$html$Html$textarea,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$rows(25),
								$elm$html$Html$Attributes$id('latex'),
								$elm$html$Html$Attributes$readonly(true),
								$elm$html$Html$Attributes$class('form-control')
							]),
						_List_fromArray(
							[
								$elm$html$Html$text(model.latexOut)
							]))
					]))
			]));
};
